...
17. **I13 Stage 2 retained-candidate finalization — technically complete.**
    ...
    reconstruction or another local Git write, issue one exact old-head force-
...
18. **Policy-driven [DCP Lab happy path v1](DCP_LAB_HAPPY_PATH_V1_CONTRACT.md)
    ...
    source PRs #39/#40/#41/#42 are reviewed, green and merged; exact repair source
...
20. **Admission wake and Human Gate UI v1 — technically qualified.**
    ...
    review and green source/package, was pinned and installed, then its first
...
22. **First real repo-only target v1 — technically complete, curator live task pending.** Exact
    ...
    merged at `f94b0603...`, tree `11a9856e...`. Pin PR #199 and bounded
    provider-query correction PR #200 merged; repeated stopped deterministic
    install/preflight passed with receipt `06ebdbf6...`. SQLite/history stayed
    unchanged, the new target has zero task/session/model activity and the app
...
26. **`wb-core` repo-only Release Train handoff — technically complete.** Exact
   ...
   The predecessor correctly kept submit closed before mutation while the
   marker was absent. WBC PR #984 and Release Train run `32030649900` published
   `wb-core.dcp-release-handoff/v1`, no-auto-sync/fresh-readmission semantics and
   exact terminal proof at WBC main `4735f74a...`. Managed-source PR #62
   ...
   deterministic receipt `97c4b6c0...` remain exact. Installed preflight and
   the direct gate are `qualified`; SQLite remains `2363c7ed...`, with zero WBC
   task/session rows and zero queued/active model actions. The exact installed
   app/daemon are currently running and healthy; no restart cause is asserted.
...
27. **`wb-core` CI truth and lifecycle UX — installed; canary blocked on fresh
   readmission.** Correct exact-head CI selection to the configured `baseline`,
   ...
   tree `828c3c6b...`; pin PR #227 merged and deterministic install produced
   receipt `44a6a690...`. The same canary reached one approved review,
   admission and Release Train handoff. Concurrent WBC main advance caused the
   repository-owned readmission marker to stop it correctly at
...
28. **`wb-core` full release/deploy path — reviewed source/pin; install pending.** Add a
   ...
   `22d8a6a474...`, tree `38ad8eabee...`; pin PR #233 and receipt `e3bda8d3...`
...
29. **WBC integration twin and DCP v2 — Stage 1 architecture authorized.**
   ...
   Stage 2 is now technically `COMPLETE`; exact repository/ruleset/environment,
   PR/check/review/merge, artifact/proof, persistent-service, host and protected
   co-tenant facts are in
   `DCP_WBC_INTEGRATION_TWIN_STAGE2_TERMINAL_EVIDENCE.md`. The combined forward
   authority is
   `DCP_WBC_INTEGRATION_TWIN_STAGE3_4_COMBINED_EXECUTION_CONTRACT.md`: complete
