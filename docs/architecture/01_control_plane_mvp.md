...
The first-finished rule is deliberately conservative. The ledger can select the earliest verifier-passed candidate, but marking it `auto_promoting_first` requires an explicit policy flag. Without that flag the candidate is queued and promotion does not start. When the selected candidate later reaches `production_complete`, every other non-terminal task in the same target/epoch is marked `frozen_base_stale` or `refresh_required`; such tasks cannot be promoted until a future refresh/rebase flow creates fresh eligibility. Before a real selected promotion merges/deploys, the promotion workspace changed-file set must exactly match the verifier `changed_files`; mismatch blocks with `promotion workspace diff does not match verified diff; do not deploy`.
