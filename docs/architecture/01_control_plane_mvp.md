...
Promotion remains serial and policy-gated per target. The first safe verifier-passed candidate can become `auto_promoting_first` only when `allow_auto_first_promotion=true`; without that policy it becomes `promotion_queued`. The dispatcher skips frozen, blocked, stale and verifier-blocked candidates. Selected `Merge & Deploy` uses one backend path for UI and MCP/chat: a single selected candidate updates its own card, while multiple selected candidates create a group promotion block with deterministic planned order, overlap blockers and refresh-required entries. Managed-clone `passed` means amber `Готово к выкладке`; green `В проде` is reserved for production-complete/deployed/public-verified state. `mode=fake_complete` is allowed only as a test-safe state-machine completion and does not call the real production lane. The guarded production bridge remains disabled by default and requires both `allow_real_production_promotion=true` and server-side enablement/stub mode before it can do anything beyond a blocked decision.
