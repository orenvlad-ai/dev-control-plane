...
- `POST /api/parallel-tasks/{id}/promote` and `POST /api/parallel-targets/{id}/promote-next` evaluate promotion state and can fake-complete the state machine for tests only;
...
- MCP write tools `submit_parallel_task`, `start_parallel_task_execution`, `reconcile_parallel_task`, `promote_parallel_task` and `promote_next_parallel_candidate` require OAuth `dcp.write`.
