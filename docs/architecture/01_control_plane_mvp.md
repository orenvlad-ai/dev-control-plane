...
- TaskSpec contracts for bounded runs.
...
- Operator-confirmed managed Codex runs in managed clones through CLI and the direct MCP `start_wb_core_auto_task` path.
- Hosted live monitor timeline and terminal output for real Codex runs.
- Hosted monitor at `/runs/live`, with sanitized terminal-like output, stage timeline, active/recent runs, changed files, verifier status and final report/handoff previews.
- Runner CLI for prepare-run, fake run-step, verify-run and cleanup-run.
- Runner CLI and direct MCP path for gated real Codex target runs using managed clone workspaces.
- MCP Stage 1 backend at `POST /mcp` using streamable HTTP. Public ChatGPT discovery is mixed no-auth read plus OAuth-gated authenticated tools: unauthenticated discovery exposes bounded status/run/artifact/timeline/log-tail/target tools only, while authenticated OAuth sessions can see `start_wb_core_auto_task`, safety-only `request_rollback` and read-only target documentation tools. OAuth supports `offline_access` refresh-token rotation with hash-only storage and family revocation on reuse.
- Legacy sprint/ping-pong/parent-child/parallel/promotion/managed-clone-only MCP orchestration is removed from the operator runtime.
...
3. `Мониторинг` links to `/runs/live`, which uses the same visual shell and continues to render sanitized terminal output, timelines, changed files, handoff/report details and direct-run production evidence. It does not expose selected promotion, sprint parent/child, parallel queue or managed-clone fallback controls.
