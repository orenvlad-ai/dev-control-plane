...
The hosted live monitor follows the same run model. `/runs/live` and `/runs/<run_id>/watch` render sanitized state only and are protected by Basic Auth. The API/SSE endpoints under `/api/runs/*` expose sanitized summaries, timeline events and terminal tails only; they do not expose raw logs or command input. ANSI handling allowlists SGR color/style codes and strips unsafe terminal controls such as OSC/DCS/clipboard/title/hyperlink sequences.
