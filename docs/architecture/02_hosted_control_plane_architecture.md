...
This is an architecture and governance document. The current implementation covers the local/hosted-ready filesystem state foundation, loopback-only hosted server runtime foundation, dev-control-plane repo self-closure policy, remote managed target source, decision-only target PR/preview/approval gates, an explicit `wb-core` production lane, and a separate `wb-core` operator-parity lane for preflighted runtime/archive/browser/session investigation. It does not authorize public routes beyond the approved dev-control-plane host, production deploy for non-`wb-core` targets, external WB live actions, uncontrolled database/data mutations or secrets handling changes.
...
The parallel task ledger collection is a machine-readable MVP for future multi-source orchestration. It is scoped by `target_id + promotion_epoch`, accepts tasks from multiple chats/sources with optional batch/release metadata, and records only lifecycle state and run bindings. The operator dashboard shows task/candidate/promotion summaries and refresh-required/frozen states. The server API exposes `POST/GET /api/parallel-tasks`, explicit execution/reconcile/promote actions, and target promotion state/candidate reads; MCP exposes the same bounded surface with write actions behind OAuth `dcp.write` plus sanitized read tools. Default execution is fake/state-only. Guarded `real_managed_clone` is an explicit bridge into the existing managed-clone runner, disabled unless runtime policy and caller confirmation allow it. Selected production is explicit and hosted-only: `promote_parallel_selection` must receive confirmation/real-promotion policy, apply selected managed-run artifacts to a fresh current-main managed workspace, re-run verifier checks, and then call the existing wb-core production lane with its PR/merge/deploy/probe gates. This layer must not call the sprint/ping-pong curator path or bypass the production lane.

## Managed Workspaces

Codex execution must happen only inside a managed clone or managed workspace owned by the control-plane. The original target repo working tree is read-only context. The explicit `wb_core_operator_parity_task` route is the bounded exception: it uses a configured persistent server-side operator worktree, not the product deploy directory, after capability preflight and still cannot PR/merge/deploy without the normal production gates.
...
The current general implementation provides decision-only target PR planning in `src/dev_control_plane/target_workflow.py`, runner commands and server endpoints. The plan uses branch names of the form `devcp/<run_id>-<slug>`, Russian commit/PR text, a required PR description, verifier result, changed files, preview URL and rollback/close instructions. The explicit `wb-core` production lane in `src/dev_control_plane/target_production.py` is the exception: it may execute GitHub PR creation/merge and production deploy only after verifier, secrets, forbidden-path, rollback, GitHub CLI availability, hosted GitHub auth readiness, hosted wb-core deploy SSH readiness and head-SHA gates pass. The server-side sprint implementation is frozen for operator flow and is not a target apply path: ordinary ChatGPT Project WebCore work must not create `mcp-sprint-*` parents, child managed-run cards or curator ping-pong steps.
...
- Read/status: `get_status`, `list_targets`, `get_target_status`, `get_operator_parity_status`, `get_production_lock_status`, `list_active_runs`, `get_run_status`, `get_run_report`, `get_run_timeline`, `get_run_log_tail`, `list_run_artifacts`, `get_run_artifact`, `get_rollback_plan`, `list_parallel_tasks`, `get_parallel_task`, `get_target_promotion_state`, `list_parallel_candidates`, `search`, `fetch`.
...
- Write/gated operator tools: `start_wb_core_auto_task`, `start_wb_core_operator_parity_task`, `merge_deploy_ready_run`, `start_wb_core_production_lane`, `start_managed_clone_run`, `submit_parallel_task`, `start_parallel_task_execution`, `reconcile_parallel_task`, `promote_parallel_task`, `promote_next_parallel_candidate`, `promote_parallel_selection`, `clear_wb_core_promotion_queue`, `archive_wb_core_auto_task_run`, `resume_wb_core_production_deploy`, `request_rollback`. These are hidden from public no-auth `tools/list` and are visible only after OAuth authorization-code + PKCE grants the `dcp.write` scope, or to legacy bearer-auth protocol smoke clients. `start_sprint` is registered only as a frozen/internal compatibility surface and is hidden from ordinary operator discovery.

The MCP layer is an adapter over existing control-plane code. It must not duplicate production-lane deploy logic. `start_wb_core_auto_task` is the normal ChatGPT Project route for `wb-core`/WebCore work. When no production-capable run or production lock is active, `wb_core_exclusive_auto_production` runs one clean managed clone through Codex, verifier and the existing guarded wb-core PR/merge/deploy/probe lane. That verified managed clone is the ordinary production source of truth; `diff.patch` is audit evidence and is not re-applied in a separate promotion workspace. When another production-capable run, auto-production intent or lock is active, the server returns `wb_core_direct_auto_blocked` before creating a run, task spec, workspace or Codex process. `wb_core_prepare_only` remains legacy/explicit-review state only and is not an ordinary fallback. `start_wb_core_operator_parity_task` is separate from ordinary intake: it exists for investigations requiring runtime/archive/DB/browser/session/collector capability, exposes `get_operator_parity_status`, fails before Codex if required capabilities are missing, runs in the configured persistent operator worktree, and records sanitized preflight/broker/handoff/quarantine artifacts. Manual `Merge & Deploy` is single-run only through `merge_deploy_ready_run`; selected/group promotion state is legacy/admin-only and must not block ordinary intake. The arbiter must never choose sprint, parent/child decomposition, `start_managed_clone_run`, `DEVCONTROL_START_SPRINT_V1` bridge or managed-clone-only fallback for ordinary WebCore work. `start_wb_core_production_lane` remains the explicit production/apply/deploy tool. `resume_wb_core_production_deploy` remains the only recovery path for an already merged blocked production-lane run and never reruns Codex or changes the diff.
...
- OAuth protected-resource and authorization-server metadata are public under `/.well-known/...`; dynamic client registration and token exchange are public protocol endpoints; `/oauth/authorize` inherits the reverse-proxy Basic Auth user gate. OAuth clients/codes/grants are durable runtime state collections, grants are stored as hashes, and status exposes sanitized reconnect diagnostics for unauthenticated calls, expired tokens, missing scope, missing client/grant and resource metadata mismatch. Tokens, Authorization headers, cookies, raw provider bodies and secret material are never logged or returned outside protocol-required responses. ChatGPT connector/link-cache failures such as transient `404 Link not found` remain external, but DevControl reports the server-side OAuth/resource/grant state needed to diagnose them.
...
Monitoring cards use operator lifecycle semantics rather than raw run color. Direct verifier-passed production-capable work is amber `Готово к выкладке`; green `В проде` is reserved for production-complete / deployed / public-verified state. Historical sprint parent and child runs are read-only archival cards and are never selectable for promotion. Cards show short human task titles, compact timing, changed-file hints, blockers and production linkage, with long ids visible only as secondary detail. Ordinary `Merge & Deploy` is single-run only through `merge_deploy_ready_run`: it accepts exactly one `ready_for_single_merge_deploy` run, re-verifies the same managed clone/worktree, then enters the existing wb-core PR/merge/deploy/probe lane without diff.patch transport. The UI may keep checkbox affordances, but once one ready run is selected, other eligible runs are disabled; backend requests with more than one id fail closed and create no promotion group. Legacy selected-promotion groups are admin/audit state only, may be read, cancelled, expired or archived, and must not block ordinary `start_wb_core_auto_task` intake. True verifier/deploy/security/GitHub/SSH blockers remain red. Local/test profiles stay fail-closed or stubbed.
...
The current sprint loop is intentionally MVP-bounded rather than a general server-side autonomy system. It supports only `target_id=wb-core`, `execution_mode=managed_clone_only`, bounded `max_steps` and `max_retries_per_step`, and artifacts/reporting visible in `Мониторинг`. Future scope may add richer dynamic sprint planning and broader target/mode support, but production-lane work remains outside sprint and must use the explicit production-lane tools and gates.
