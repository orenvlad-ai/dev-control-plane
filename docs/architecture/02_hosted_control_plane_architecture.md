...
Monitoring cards use operator lifecycle semantics rather than raw run color. Managed-clone `passed` / verifier-passed state is amber `Готово к выкладке` and selectable for selected `Merge & Deploy`; green `В проде` is reserved for production-complete / deployed / public-verified state. Cards show compact timing, changed-file hints, blockers and production linkage. Selected `Merge & Deploy` is available through the same backend path from UI and MCP/chat (`promote_parallel_selection`): one selected item continues inside its original card, while two or more selected items create a group promotion block with planned order, per-item status, blockers and refresh-required entries. Real production bridge remains disabled by default and fail-closed unless explicit policy and server-side enablement are present.
