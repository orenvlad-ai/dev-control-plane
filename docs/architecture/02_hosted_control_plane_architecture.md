...
- Web/UI service: unified dark dashboard shell with `Панель`, `Подключение`, `Живые запуски` and `Технические детали` sections. Legacy chat/curator/task-card APIs may remain backend compatibility paths, but they are not the primary hosted UI.
...
The current sprint loop is intentionally MVP-bounded rather than a general server-side autonomy system. It supports only `target_id=wb-core`, `execution_mode=managed_clone_only`, bounded `max_steps` and `max_retries_per_step`, and artifacts/reporting visible in `Живые запуски`. Future scope may add richer dynamic sprint planning and broader target/mode support, but production-lane work remains outside sprint and must use the explicit production-lane tools and gates.
