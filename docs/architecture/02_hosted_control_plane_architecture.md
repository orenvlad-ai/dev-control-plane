...
- Write/gated operator tools: `start_wb_core_auto_task`, `start_wb_core_production_lane`, `start_managed_clone_run`, `submit_parallel_task`, `start_parallel_task_execution`, `reconcile_parallel_task`, `promote_parallel_task`, `promote_next_parallel_candidate`, `promote_parallel_selection`, `resume_wb_core_production_deploy`, `request_rollback`. These are hidden from public no-auth `tools/list` and are visible only after OAuth authorization-code + PKCE grants the `dcp.write` scope, or to legacy bearer-auth protocol smoke clients. `start_sprint` is registered only as a frozen/internal compatibility surface and is hidden from ordinary operator discovery.

The MCP layer is an adapter over existing control-plane code. It must not duplicate production-lane deploy logic. `start_wb_core_auto_task` is the normal ChatGPT Project route for `wb-core`/WebCore work: the server checks current non-terminal wb-core runs, active auto intents, production/promotion locks, deferred selected candidates and resume/deploy state, then either starts exactly one direct production-capable route before Codex starts or returns a precise blocker. `wb_core_exclusive_auto_production` runs managed clone/Codex, verifier and the existing guarded wb-core PR/merge/deploy/probe lane; `wb_core_direct_auto_blocked` returns before Codex starts when state is busy, uncertain or gate-blocked. The arbiter must never choose sprint, parent/child decomposition, `start_managed_clone_run`, `DEVCONTROL_START_SPRINT_V1` bridge or managed-clone-only fallback for ordinary WebCore work. `start_wb_core_production_lane` remains the explicit production/apply/deploy tool and uses the existing managed-clone/Codex path and then the existing `execute_wb_core_production_lane` path when a real non-dry run is allowed. Dry-run starts create isolated artifacts and rollback-plan evidence without calling Codex, GitHub, SSH or WebCore deploy. `start_managed_clone_run` is managed-clone-only and cannot PR, merge or deploy; it is not a fallback for WebCore tasks that expect merge/deploy. `start_sprint`, the sprint orchestrator, curator-to-Codex loop and `DEVCONTROL_START_SPRINT_V1` compatibility bridge are frozen: non-internal calls return `start_sprint is frozen for operator flow; use direct wb-core auto Codex task` and create no parent/child run tree. `resume_wb_core_production_deploy` is the only recovery path for an already merged blocked `wb-core` production-lane run; it requires a recorded verifier pass, PR URL/number, merge commit, rollback plan, allowed changed files, GitHub auth, SSH readiness and target lock, then resumes backup/deploy/probes only. It never reruns Codex, changes the diff, creates a branch, pushes, opens a new PR or merges again. Start/resume responses include `live_url` and `watch_url` so the hosted operator can follow the run in the live monitor.
