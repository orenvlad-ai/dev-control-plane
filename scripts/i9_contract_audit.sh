...
grep -Fq 'Real-target repository rename v1 — technically complete.' docs/ROADMAP.md
...
grep -Fq 'bc49040398a05c6127b140cd10f3828db178bc410f1b722f887ae7d63b79438b' docs/DCP_REAL_TARGET_REPOSITORY_RENAME_V1_TERMINAL_EVIDENCE.md
grep -Fq 'DCP_REAL_TARGET_REPOSITORY_RENAME_V1_TERMINAL_EVIDENCE.md' AGENTS.md docs/CURRENT_OPERATING_CONTRACT.md docs/PROJECT_BRIEF.md docs/ROADMAP.md docs/DECISIONS.md
...
grep -Fq '1335072844' "$real_target"
grep -Fq '`wb-price-extension`' "$real_target"
grep -Fq '`repo-only`' "$real_target"
grep -Fq 'required check name is exactly `baseline`' "$real_target"
grep -Fq 'Both exact targets share the existing `dcp_model_action` FIFO' "$real_target"
grep -Fq 'render neither internal heading nor empty section' "$real_target"
grep -Fq 'preparation task launches no worker, reviewer or arbiter' "$real_target"
for policy_file in AGENTS.md docs/CURRENT_OPERATING_CONTRACT.md docs/PROJECT_BRIEF.md docs/ROADMAP.md docs/DECISIONS.md; do
	grep -Fq 'DCP_REAL_TARGET_V1_CONTRACT.md' "$policy_file"
done

provider_identity=docs/DCP_REAL_TARGET_PROVIDER_IDENTITY_V1_CONTRACT.md
[[ -s "$provider_identity" ]]
grep -Fq 'contract_status: owner-approved-pre-runtime' "$provider_identity"
grep -Fq 'Unknown JSON field: "databaseId"' "$provider_identity"
grep -Fq '`1335072844`' "$provider_identity"
grep -Fq '`237411244`' "$provider_identity"
grep -Fq 'typed Go value' "$provider_identity"
grep -Fq 'Do not run the real product DCP task' "$provider_identity"
for policy_file in AGENTS.md docs/CURRENT_OPERATING_CONTRACT.md docs/PROJECT_BRIEF.md docs/ROADMAP.md docs/DECISIONS.md; do
