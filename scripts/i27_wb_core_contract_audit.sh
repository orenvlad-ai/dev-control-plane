...
grep -Fq 'technical status is therefore `BLOCKED` pending reviewed adapter merge' "$contract"
grep -Fq 'current_readiness_status: blocked-project-identity-reconciliation' "$contract"
...
grep -Fq 'operating_contract_revision: 2026-08-17.4' "$current"
