...
SQL
dcp_ao_install_assert_no_active_model_actions "$lab_root"
sqlite3 "$database" "UPDATE dcp_review_lab_card12_cold_start_recovery SET status='backed_up';"
if dcp_ao_install_assert_no_active_model_actions "$lab_root"; then exit 1; fi
sqlite3 "$database" "UPDATE dcp_review_lab_card12_cold_start_recovery SET status='running', model_free_action_count=1;"
if dcp_ao_install_assert_no_active_model_actions "$lab_root"; then exit 1; fi
sqlite3 "$database" "UPDATE dcp_review_lab_card12_cold_start_recovery SET status='candidate_ready';"
if dcp_ao_install_assert_no_active_model_actions "$lab_root"; then exit 1; fi
sqlite3 "$database" "UPDATE dcp_review_lab_card12_cold_start_recovery SET status='review_running', reviewer_model_call_count=1;"
if dcp_ao_install_assert_no_active_model_actions "$lab_root"; then exit 1; fi
sqlite3 "$database" "UPDATE dcp_review_lab_card12_cold_start_recovery SET status='failed';"
dcp_ao_install_assert_no_active_model_actions "$lab_root"
