...
- The MCP Stage 1 bridge is a bounded backend interface at `POST /mcp` using streamable HTTP. It must stay behind the public auth boundary and write tools must also require separate MCP bearer auth or a future OAuth gate.
...
- ChatGPT Developer Mode setup currently has an auth blocker for write tools because the repo implements bearer-token write auth, while current OpenAI app setup docs document OAuth/No Auth/Mixed Auth. Do not remove the bearer gate or make write tools unauthenticated to work around this.
