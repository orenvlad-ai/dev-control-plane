"""Repo-only contract skeleton for the server development control-plane MVP."""
...
class ControlPlaneValidationError(ValueError):
    """Raised when a development control plane contract violates deterministic policy."""
...
def validate_task_spec(task_spec: TaskSpec, require_frozen: bool = False) -> None:
    ...
    if require_frozen and task_spec.status != "frozen":
        raise ControlPlaneValidationError("run/prompt generation requires a frozen task spec")
    if task_spec.task_class == "L3" and not task_spec.human_gates and not _is_present(task_spec.explicit_policy_note):
        raise ControlPlaneValidationError("L3 task spec requires a human gate or explicit policy note")
    if "execution_from_discussion" not in task_spec.forbidden_actions:
        raise ControlPlaneValidationError("execution_from_discussion must stay forbidden")


def validate_sprint_step(step: SprintStep) -> None:
    _require_non_empty("sprint_step.id", step.id)
    if step.sequence < 1:
        raise ControlPlaneValidationError("sprint_step.sequence must be >= 1")
...
def validate_sprint_plan(plan: SprintPlan, task_spec: TaskSpec) -> None:
    ...
    if plan.task_spec_id != task_spec.id:
        raise ControlPlaneValidationError("sprint_plan.task_spec_id must match task_spec.id")
    if not plan.steps:
        raise ControlPlaneValidationError("sprint_plan.steps must not be empty")

    seen_sequences: set[int] = set()
    for step in plan.steps:
        validate_sprint_step(step)
        if step.sequence in seen_sequences:
            raise ControlPlaneValidationError(f"duplicate sprint_step.sequence: {step.sequence}")
...
def build_codex_prompt(
    ...
    lines = [
        ...
        *_format_items(sprint_step.stop_conditions),
        ...
        "For repo `orenvlad-ai/dev-control-plane` only, Codex may commit, push, open a PR, merge its own PR and delete its own branch, including L3 tasks, when every merge eligibility gate is clean.",
        "This closure right is limited to a PR created for the current task or an explicitly current `codex/*` branch in dev-control-plane.",
        "Before merge, verify: clean `git status --short --branch`, open PR, expected head SHA match, required checks/smokes passed, `git diff --check`, `git diff --cached --check`, verifier passed, forbidden paths/actions absent, protected derived docset paths untouched unless this is an explicit derived-sync task, secrets scan clean, final handoff complete, blocker absent, and no explicit `NO_AUTO_MERGE` instruction.",
        "This right does not apply to target repositories, including `wb-core`; target repo PR/merge/apply needs a separate explicit apply policy.",
        "This right does not allow production deploy, preview/staging deploy, public routes, SSH/root, direct target mutation, or bypassing verifier/checks.",
        "If any gate fails, leave the PR open and report the exact blocker.",
        "",
        "## Mandatory Final Handoff Contract",
        'Your final answer MUST start with the exact first line: "=== ДЛЯ КУРАТОРА ===".',
        'Do not start the final answer with "Статус:" or any other heading.',
        'The final answer MUST also contain the exact line: "=== СЖАТАЯ ПРОВЕРКА ===".',
...
def _require_non_empty(label: str, value: str | None) -> None:
    if not _is_present(value):
        raise ControlPlaneValidationError(f"{label} is required")
...
def _require_non_empty_sequence(label: str, values: Sequence[str]) -> None:
    if not values or not any(_is_present(value) for value in values):
        raise ControlPlaneValidationError(f"{label} must not be empty")


def _require_status(label: str, value: str, allowed: set[str]) -> None:
    if value not in allowed:
        raise ControlPlaneValidationError(f"{label} must be one of {sorted(allowed)}")


def _require_task_class(value: str) -> None:
    if value not in TASK_CLASSES:
        raise ControlPlaneValidationError("task_class must be one of L1, L2, L3")
...
def _require_required_defaults(label: str, values: Sequence[str], defaults: Sequence[str]) -> None:
    ...
    if missing:
        raise ControlPlaneValidationError(f"{label} missing required defaults: {missing}")
...
def _reject_action_overlap(allowed_actions: Sequence[str], forbidden_actions: Sequence[str]) -> None:
    ...
    if overlap:
        raise ControlPlaneValidationError(f"allowed_actions overlap forbidden_actions: {overlap}")
...
def _mapping_optional_str(payload: Mapping[str, Any], key: str) -> str | None:
    ...
    if value is None:
        return None
    if not isinstance(value, str):
        raise ControlPlaneValidationError(f"{key} must be a string when provided")
    return value
...
def _mapping_int(payload: Mapping[str, Any], key: str) -> int:
    ...
    if isinstance(value, bool) or not isinstance(value, int):
        raise ControlPlaneValidationError(f"{key} must be an integer")
    return value
