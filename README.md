...
Operator-parity runs are a separate hosted lane, not a production apply path. They use the configured persistent wb-core operator worktree under state/runtime policy and expose a sanitized capability matrix through `get_status`, `get_target_status` and `get_operator_parity_status`. Runtime/archive access is read-only by default through allowlisted live paths such as `promo_campaign_archive`, `promo_xlsx_collector_runs`, `registry_upload_runtime.sqlite3` and `seller_portal_relogin`, or through a sanitized broker export. Any production follow-up from a parity investigation must enter the normal guarded wb-core PR/merge/deploy lane.
