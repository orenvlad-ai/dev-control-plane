...
Monitoring cards use an operator lifecycle mapping instead of raw status colors. Managed-clone `passed` / verifier-passed output is amber `Готово к выкладке`, not green production success; green is reserved for `production_complete` / deployed / public-verified outcomes. Cards show compact start/finish/duration text, changed-file hints and promotion eligibility. Eligible cards can be selected and sent through one `Merge & Deploy` action. A single selected item continues promotion state on its own card; multiple selected items create a group promotion block with planned order, blockers and refresh-required entries.
