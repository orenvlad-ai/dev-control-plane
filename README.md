...
Monitoring cards use an operator lifecycle mapping instead of raw status colors. Managed-clone `passed` / verifier-passed output is amber `Готово к выкладке`, not green production success; green is reserved for `production_complete` / deployed / public-verified outcomes. Cards show short human task titles first, with long `run_id`/`task_id` kept as secondary detail, plus compact start/finish/duration text, changed-file hints and promotion eligibility. Eligible cards can be selected and sent through one `Merge & Deploy` action. A single selected item continues promotion state on its own card; if the real bridge is unavailable it records a visible blocker instead of a no-op. Multiple selected items create a persisted group promotion block with planned order, blockers and refresh-required entries. Stale `plan_ready` group blocks are reconciled to blocked/expired/cancelled terminal states and group `Stop` updates backend state rather than leaving a blinking UI-only object.
...
Before Codex starts, the runner applies a prompt consistency gate for contradictory envelopes such as production-lane requests that also say repo-only/no-live/no-deploy, UI work that says no UI, or Codex execution that says no Codex worker run. Conflicts return a controlled blocker and record the conflicting phrases instead of starting Codex.
...
The execution coordinator foundation is explicit and guarded. `start_parallel_task_execution` defaults to fake/state-only binding and moves a ledger task to `managed_run_running`; `execution_mode=real_managed_clone` is disabled unless the runtime explicitly enables it and the caller confirms the bridge into the existing managed-clone runner. It never calls `start_sprint`, PR, merge, deploy or production-lane work. `reconcile_parallel_task` can consume explicit status/report data or read sanitized existing run/job artifacts and marks the task `verifier_passed`, `blocked` or `failed`. `promote_parallel_task`, `promote_next_parallel_candidate` and `promote_parallel_selection` evaluate or fake-complete promotion state. `promote_parallel_selection` is the MCP/chat handle for the same selected `Merge & Deploy` flow as the UI; it resolves task ids, candidate ids or verifier-passed managed run ids into a deterministic single/group plan. Group conflict detection is embedded in that flow, not exposed as a separate button: non-overlapping selected candidates become accepted tasks, same-group overlaps become deferred `ready_for_separate_deploy` tasks, and a group that deploys accepted tasks while deferring conflicts ends as warning-level `partially_deployed`, not a red failure. Deferred tasks remain selectable for a later separate `Merge & Deploy`; true verifier/deploy/forbidden-path/secrets/GitHub/SSH blockers stay red. Production promotion never starts without explicit policy: the first-finished candidate can become `auto_promoting_first` only with `allow_auto_first_promotion=true`, and selected real production additionally requires `confirm_merge_deploy=true`, `allow_real_production_promotion=true`, hosted/runtime bridge enablement and the existing wb-core production-lane gates. In hosted mode the selected managed-run adapter applies the selected diff to a fresh current `main` managed workspace, re-runs verifier checks, then hands that workspace to the existing PR/merge/deploy/probe lane; local/test profiles remain fail-closed or stubbed. `mode=fake_complete` remains a test-safe state-machine completion that does not call the real production lane. `list_parallel_candidates` exposes sanitized candidate state and blockers.
