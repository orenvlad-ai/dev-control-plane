"""Local repo-only CLI for development control-plane MVP contract tooling."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Repo-only development control-plane MVP contract tooling.")
...
def _freeze_task_spec_summary(input_path: Path, output_path: Path, frozen_at: str | None) -> tuple[dict[str, Any], int]:
    ...
    if task_spec.status != "draft":
        raise ControlPlaneValidationError("freeze-task-spec expects a draft task spec")
...
def _read_json(path: Path) -> Mapping[str, Any]:
    ...
    if not isinstance(payload, Mapping):
        raise ControlPlaneValidationError("JSON root must be an object")
    return payload
